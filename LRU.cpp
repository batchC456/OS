...
int main(){
    ...
    for(int i=0; i<n; i++){
        ...
        if(flag==0){
            ...
            vector<int> minidxarr(fs, -1); //Array to find minimum index of each page in the Queue
            ...
            //Finding minimum index  of page in : minidx array
            int minidx = i;
            int pageidx = -1;

            for(int j=0; j<fs; j++){
                if(minidxarr[j] < minidx){
                    minidx = minidxarr[j];
                    pageidx = j;
                }
            }

            //Replace the LRU Page with New Page in the Queue
